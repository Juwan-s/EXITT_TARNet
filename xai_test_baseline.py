...
from captum.attr import Saliency, InputXGradient, DeepLift, IntegratedGradients, GuidedBackprop
...
parser.add_argument('--xai_method', type=str, default='LRP', help='[GI, DeepLift, Saliency, LRP]')
...
class Attn_Max_Last:
    ...
    def attribute(self, x, target):
        ...
        layer_idxs = attention_probs.keys()
        R = np.expand_dims(np.max([x_.sum(1) for x_ in attention_probs[max(layer_idxs)].detach().cpu().numpy()], 1), axis=-1) 
        return R
...
def evaluate(y_pred, y, nclasses, criterion, task_type, device, avg):
    ...
    if task_type == 'classification':
        loss = criterion(y_pred.view(-1, nclasses), torch.as_tensor(y)).item()
...
def perturb_zero(model, X, y, batch, nclasses, xai_method_1, criterion, task_type, device, avg, top_percent):
    ...
    for i in range(num_batches):
        ...
        if xai_method_1 in ['LRP', 'attn_last', 'attn_max_last', 'rollout'] :
...
def perturb_mean(model, X, y, batch, nclasses, xai_method_1, criterion, task_type, device, avg, top_percent):
    ...
    for i in range(num_batches):
        ...
        if xai_method_1 in ['LRP', 'attn_last', 'attn_max_last'] :
            relevance_scores = xai_method.attribute(input_x, target = input_y)
        else:
            relevance_scores = xai_method.attribute(input_x, target = input_y,  additional_forward_args='classification')
...
# temp = {'val': {'a': res_zero[1], 'b': res_mean[1], 'c': res_inv[1], 'd': res_avg}}
