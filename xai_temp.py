...
class MultitaskTransformerModel(nn.Module):
    ...
    def __init__(self, task_type, device, nclasses, seq_len, batch, input_size, emb_size, nhead, nhid, nhid_tar, nhid_task, nlayers, train_mode, dropout = 0.1):
        ...
        #     ptrunk = make_p_layer(nn.Linear(input_size, emb_size))
        ...
        encoder_layers = transformer.TransformerEncoderLayer(emb_size, nhead, nhid, train_mode, dropout)
    ...
    def forward_and_explain(self, x, y, task_type):
        ...
        # output = ptrunk_net(A['x_data'])
        # output = self.trunk_net(A['x_data'])
        # output = self.pproc(self.trunk_net, ptrunk_net, A['x_data'])
        ...
        class_input = norm_out[:, -1, :]

        for i, cl in enumerate(self.class_net):
            
            class_inputdata = class_input.data
            class_inputdata.requires_grad_(True)

            A['class_input_{}_data'.format(i)] = class_inputdata
            A['class_input_{}'.format(i)] = class_input

            if i == 0:
                pdense = make_p_layer(cl)
                output = self.pproc(cl, pdense, A['class_input_{}_data'.format(i)])
            ...
            if i == 6:
                ...
                output = cl(A['class_input_{}_data'.format(i)])
        ...
        for i, cl in list(enumerate(self.class_net))[::-1]:
            R_c.sum().backward()

            R_c_grad = A['class_input_{}_data'.format(i)].grad
            R_c = (R_c_grad) * A['class_input_{}'.format(i)]
            print("R_c at", i,":", R_c.sum().item())
