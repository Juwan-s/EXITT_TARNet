...
parser.add_argument('--dataset', type=str, default='multi_syn')
...
parser.add_argument('--model_path', type=str, default='./check_points/multi_syn_2048_0.001_2_128_8_0.967.mdl', help='model path')
...
def in_range_ratio(model, X, y, batch, nclasses, xai_method_1, criterion, task_type, device, avg, top_percent):
    ...
    for i in tqdm(range(num_batches)):
        ...
        # else:
        if xai_method_1 == 'GradientShap' or xai_method_1 == 'DeepShap' or xai_method_1=='IntegratedGradients':
            relevance_scores = xai_method.attribute(input_x, target = input_y,  additional_forward_args=['classification'], baselines = torch.zeros_like(input_x)).detach().cpu().numpy()
            ...
            relevance_scores = xai_method.attribute(input_x, target = input_y).numpy()
        else:
        ...
        #     mask = np.where(relevance_scores > percentiles[:, np.newaxis], 1, 0)
        #     mask = torch.from_numpy(np.broadcast_to(mask, (*mask.shape[:-1], input_x.shape[-1])))
        # else:
        mask = torch.from_numpy(np.where(relevance_scores > percentiles[:, np.newaxis], 1, 0)) # 상위 높은 애들은 1로 설정



        range_mask = fall_range[start:end]
        # range_mask = np.where(fall_range[start : end] == 1, 1, 0) # 실제 fall인 애들은 1
        # print(range_mask.shape)
        # range_mask = range_mask.reshape(num_inst, -1, 3)
        # print("range mask shape :", range_mask.shape)
        # print("mask shape :", mask.shape)
        # print("check1 :", np.isnan(range_mask).any())
        # print("check2 :", np.isnan(mask).any())
...
fall_range = np.load('./multi_syn_range.npy')

top_percent = prop['top_percent']

ap, ar = in_range_ratio(best_model, X_test[4554:], y_test[4554:], prop['batch'], prop['nclasses'], prop['xai_method'], criterion_task, prop['task_type'], prop['device'], prop['avg'], top_percent)
